...
def cuda_supports_fp16() -> bool:
    """
    Return True if the current CUDA GPU reliably supports FP16.
    Whisper FP16 is safe for compute capability >= 7.0 (Turing+).
    """
    try:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_worker_cmd(self, input_path: str, device_choice: str) -> list[str]:
        model_choice = self.combo_model.currentText()
        out_dir = os.path.dirname(input_path)

        use_fp16 = (device_choice == "cuda" and cuda_supports_fp16())

        cmd = [
            sys.executable,
            os.path.join(BASE_DIR, "worker_transcribe.py"),
            "--input", input_path,
            "--model", model_choice,
        ...
        print(f"Worker device={device_choice} | fp16={'ON' if use_fp16 else 'OFF'}")
        return cmd
    ...
    def start_batch(self):
        ...
        model_choice = self.combo_model.currentText()
        selected = self.combo_device.currentText().lower()
        device_choice = resolve_device(selected)

        # Guard: user explicitly chose CUDA, but CUDA isn't actually available
        if selected == "cuda" and (torch is None or not torch.cuda.is_available()):
            QtWidgets.QMessageBox.warning(
                self,
                "CUDA unavailable",
                "CUDA was selected but no CUDA-capable GPU is available.\n"
                "Falling back to CPU."
            )
            device_choice = "cpu"
            self.combo_device.setCurrentText("cpu")  # optional: keep UI honest
        ...
        for row, item in enumerate(self.items):
            if item.status.startswith("[CANCELLED]"):
                continue
            cmd = self._build_worker_cmd(item.path, device_choice)
            job = FileJob(
                row=row,
                cmd=cmd,
                workdir=None,
